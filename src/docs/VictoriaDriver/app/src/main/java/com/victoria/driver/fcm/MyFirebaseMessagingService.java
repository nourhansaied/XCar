...
public class MyFirebaseMessagingService extends FirebaseMessagingService {
    ...
    public void onMessageReceived(RemoteMessage remoteMessage) {
        ...
        if (remoteMessage.getData() != null) {
            ...
            if (remoteMessage.getData() != null && remoteMessage.getData().get("channel_sid") != null) {
            ...
            } else if (notification.getTag().equals(Common.PUSH_TAG_PLACE_ORDER)) {
                if (isAppIsInBackground(getApplicationContext())) {
                    sendNotification(notification, Common.PUSH_TAG_PLACE_ORDER);
                } else {
                    EventBus.getDefault().post(notification);
                }
            } else if (notification.getTag().equals(Common.PUSH_TAG_PAYMENT)) {
                notification.setTrip_id(remoteMessage.getData().get(Common.TRIP_ID));
                sendNotification(notification, Common.PUSH_TAG_PAYMENT);
                /*if (isAppIsInBackground(getApplicationContext())) {
                ...
                if (isAppIsInBackground(getApplicationContext())) {
                    sendNotification(notification, Common.PUSH_TAG_CANCEL_LATE_TRIP);
                } else {
                    EventBus.getDefault().post(notification);
                }
    ...
    public void sendNotification(Notification notification, String flag) {
        ...
        if (flag.equals(Common.PUSH_TAG_PAYMENT)) {
            intent.putExtra(Common.TRIP_ID, notification.getTrip_id());
            intent.putExtra(Common.ACTIVITY_FIRST_PAGE, flag);
        } else if (flag.equals(Common.PUSH_TAG_PLACE_ORDER)) {
            intent.putExtra(Common.ACTIVITY_FIRST_PAGE, flag);
        } else if (flag.equals(Common.PUSH_TAG_TWILLIO)) {
            intent.putExtra(Common.ACTIVITY_FIRST_PAGE, flag);
        }
