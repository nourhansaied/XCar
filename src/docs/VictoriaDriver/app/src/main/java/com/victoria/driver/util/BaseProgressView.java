...
abstract class BaseProgressView extends View {
    ...
    protected int maximum_progress = 60;

    protected Paint foregroundPaint, backgroundPaint, textPaint;
    protected float backgroundStrokeWidth = 3f, strokeWidth = 5f;
    protected int PADDING = 20;
    protected int color = Color.TRANSPARENT, backgroundColor = Color.TRANSPARENT;
    protected int textColor = getResources().getColor(android.R.color.transparent);
    protected int shadowColor = getResources().getColor(R.color.orangeBold);
    protected int textSize = 40;
    ...
    private void initTypedArray(Context context, AttributeSet attrs) {
        ...
        try {
            progress = (int) typedArray.getFloat(
                    R.styleable.MyProgress_progress, progress);
            strokeWidth = typedArray.getDimension(
                    R.styleable.MyProgress_stroke_width, strokeWidth);
            backgroundStrokeWidth = typedArray.getDimension(
                    R.styleable.MyProgress_background_stroke_width,
                    backgroundStrokeWidth);
            color = typedArray.getInt(
                    R.styleable.MyProgress_progress_color, color);
            backgroundColor = typedArray.getInt(
                    R.styleable.MyProgress_background_color, backgroundColor);
            textColor = typedArray.getInt(
                    R.styleable.MyProgress_text_color, textColor);
            textSize = typedArray.getInt(
                    R.styleable.MyProgress_text_size, textSize);
        } finally {
            typedArray.recycle();
        }
